...
import { JSONDataSource, type JSONObject } from "../JSONDataSource";

describe("JSONDataSource", () => {
	const sampleData: JSONObject[] = [
		{ id: 1, name: "Alice", value: 100, data: { status: "active" } },
		{ id: 2, name: "Bob", value: null, data: { status: "inactive" } },
	];

	it("should correctly determine column names from the first object", () => {
		const source = new JSONDataSource(sampleData);
		expect(source.getColumnNames()).toEqual(["id", "name", "value", "data"]);
	});
	...
	it("should return the correct row as an array of values", () => {
		const source = new JSONDataSource(sampleData);
		expect(source.getArrayRow(0)).toEqual([
			1,
			"Alice",
			100,
			{ status: "active" },
		]);
		expect(source.getArrayRow(1)).toEqual([
			2,
			"Bob",
			null,
			{ status: "inactive" },
